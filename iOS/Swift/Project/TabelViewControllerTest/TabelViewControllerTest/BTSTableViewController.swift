...
class BTSTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        if let imageProfile = cell.viewWithTag(1) as? UIImageView
        {
        ...
        if let lblDesc = cell.viewWithTag(3) as? UILabel
        {
            lblDesc.text = member["desc"]
        }
        
        if let imageLogo = cell.viewWithTag(4) as? UIImageView
        {
            imageLogo.image = UIImage(named: "bts-icon")
        }
        
        if let lblCompany = cell.viewWithTag(5) as? UILabel
        {
            lblCompany.text = "하이브"
        }
    ...
    {
        let selectedMember = bts[indexPath.row]
